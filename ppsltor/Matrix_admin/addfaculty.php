...
if (isset($_POST['submit'])) {
    ...
    $matrixAdminId = $_POST['matrixAdminId']; // New field
    ...
    if (move_uploaded_file($photoTmpName, $photoPath)) {
        $sql = "INSERT INTO board_members (board_member_id, board_member_name, contact_number, email_id, category_id, matrix_admin_id, photo) 
                VALUES (?, ?, ?, ?, ?, ?, ?)";

        $stmt = mysqli_prepare($con, $sql);

        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "sssssss", $boardMemberId, $boardMemberName, $contactNumber, $emailId, $categoryId, $matrixAdminId, $photo);
...
// Fetch matrix admins
$matrixAdminQuery = "SELECT * FROM matrix_admi";
$matrixAdminResult = mysqli_query($con, $matrixAdminQuery);
?>
                ...
                <div class="container card shadow p-3 bg-white rounded">
                    ...
                    <form class="mt-4" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6">
                                <label for="matrixAdminId">Matrix Admin:</label>
                                <select class="form-control" id="matrix_admin_id" name="matrix_admin_name" required>
                                    <option value="">-- Select Admin --</option>
                                    <?php while($admin = mysqli_fetch_assoc($matrixAdminResult)): ?>
                                        <option value="<?= $admin['matrix_admin_id']; ?>"><?= $admin['matrix_admin_name']; ?></option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
                        </div>
